...
from gp2.utils.util import Util
...
class BaseCNNDiscriminator(Discriminator, ABC):
    ...
    def train(
        ...
        """
        super().train(
            X_train_images, X_train_masks, y_train, X_val_images, X_val_masks, y_val
        )
        checkpoint_file = os.path.join(self.workingdir, "cnnd")
        checkpoint_file = Util.create_numbered_file(checkpoint_file, ".model")

        callbacks = [
            EarlyStopping(patience=patience_counter, monitor="loss", verbose=0),
            ModelCheckpoint(
        ...
        history = self.model.fit(
            x=[X_train_images, X_train_masks],
            y=y_train,
            batch_size=batch_size,
            epochs=epochs,
            callbacks=callbacks,
            validation_data=([X_val_images, X_val_masks], y_val),
            verbose=0,
        )
...
class CustomMaskLayer(Layer):
    ...
    def __init__(self, mask_trainable=False, **kwargs):
    ...
    def call(self, inputs, *args, **kwargs):
        ...
        Returns:
            the masked inputs
        """
        mask_cast = tf.cast(self.mask, dtype=inputs.dtype)
        return inputs * mask_cast
    ...
    def set_mask(self, mask) -> None:
        ...
        Returns:
            None
        """
        mask_tensor = tf.convert_to_tensor(mask, dtype=self.mask.dtype)
        self.mask.assign(mask_tensor)
