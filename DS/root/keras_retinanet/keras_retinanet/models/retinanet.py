...
def create_context_extractor():
    ...
    features = keras.layers.Input(shape=(None, None, None))
    pooled_features = keras.layers.GlobalAveragePooling2D()(features)
    zerod_features = keras.layers.Lambda(lambda x: x*0)(features)
    pooled_features_broadcast = keras.layers.Lambda(lambda x: x[0] + x[1])([pooled_features, zerod_features])
    context_features_concat = keras.layers.Concatenate()([features, pooled_features_broadcast])

    return keras.models.Model(input=features, output=context_features_concat)
...
def retinanet(
    ...
    if global_pool:
        context_extractor_model = create_context_extractor()
        features = [context_extractor_model(feature) for feature in features]
        print('\033[1m \033[93m Added global pooling to features \033[0m')
