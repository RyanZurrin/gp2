...
def compute_bb_targets(image_group, annotations_group, num_classes, batch_size=1,
                       **kwargs):
    ...
    for index, (image, annotations) in enumerate(
        ...
        regression_group[index] = np.append(regression_group[index], anchor_states,
                                            axis=1)

    labels_batch = np.zeros((batch_size,) + labels_group[0].shape,
                            dtype=keras.backend.floatx())
    regression_batch = np.zeros((batch_size,) + regression_group[0].shape,
                                dtype=keras.backend.floatx())
...
def create_model_ensemble(model_list, nms=True, score_threshold=0.00,
                          max_detections=6, nms_threshold=0.00,
    ...
    for model_idx, model in enumerate(model_list):
        models_features.append(
            [model.get_layer(p_name).output for p_name in ['model_%d_P3' % model_idx,
    ...
    if zero_benigns:
        ...
        z_weights[1, 1] = 1.0
        benign_filter_layer = keras.layers.TimeDistributed(keras.layers.Dense(2),
                                                           weights=[z_weights,
                                                                    np.zeros(2)])
    ...
    detections = layers.FilterDetections(nms=nms, name='filtered_detections',
                                         score_threshold=score_threshold,
                                         max_detections=max_detections,
                                         nms_threshold=nms_threshold)(
        [boxes, avg_classification])
    outputs = detections
    new_model = keras.models.Model(inputs=[model.inputs[0] for model in model_list],
                                   outputs=outputs, name='model_ensemble')
